...
class BiomarkerTrendLine extends StatelessWidget {
  ...
  final List<BiomarkerTrendLinePoint> points;
  ...
  const BiomarkerTrendLine({
    ...
    this.maxPointsToDisplayCount = 3,
    required this.valueTextStyle,
    required this.dateTextStyle,
    required this.hiddenPointsCountTextStyle,
  })  : assert(points.length >= 2),
        assert(maxPointsToDisplayCount >= 2),
        super(key: key);
...
class _Painter extends CustomPainter {
  ...
  late final List<BiomarkerTrendLinePoint> points;
  late final List<BiomarkerTrendLinePoint> pointsToDisplay;
  final int maxPointsToDisplayCount;
  ...
  _Painter({
    required List<BiomarkerTrendLinePoint> points,
  ...
  List<_Point> _computePoints(Size canvasSize) {
    ...
    return List<_Point>.generate(
      ...
      (index) {
        ...
        return _Point._fromBiomarkerTrendLinePoint(
...
@immutable
class BiomarkerTrendLinePoint implements Comparable<BiomarkerTrendLinePoint> {
  final double value;
  final DateTime time;
  final Color color;

  const BiomarkerTrendLinePoint({
    required this.value,
    required this.time,
  ...
  @override
  int compareTo(BiomarkerTrendLinePoint other) {
    if (other.time == time) {
      return 0;
    }
  ...
  bool operator ==(Object other) =>
      ...
      other is BiomarkerTrendLinePoint &&
...
class _Point {
  ...
  factory _Point._fromBiomarkerTrendLinePoint(
    BiomarkerTrendLinePoint point,
    Offset position,
  ) =>
      _Point(
        value: point.value,
